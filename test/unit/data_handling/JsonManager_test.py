...
class JsonManagerTest(unittest.TestCase):
    ...
    def test_assert_false_valid_path(self):
        self.assertFalse(JsonManager.is_valid_path(
                         JsonManager.get_path(Setting.SYNTHETIC_DATA_INVALID_TYPE)),
    ...
    def test_assert_true_valid_json(self):
        for abs_type in AbstractionType:
            ...
            if abs_type is AbstractionType.CUSTOMER:
                json_to_try = JsonManager.yield_json(
                              AbstractionType.CUSTOMER, self.customer)
            elif abs_type is AbstractionType.HOTEL:
                json_to_try = JsonManager.yield_json(
                              AbstractionType.HOTEL, self.hotel)
            elif abs_type is AbstractionType.RESERVATION:
                json_to_try = JsonManager.yield_json(
                              AbstractionType.RESERVATION, self.reservation)

            self.assertTrue(JsonManager.is_valid_json(json_to_try),
                            msg=f"Testing if JSON for {abs_type} is Valid")

    def test_assert_false_valid_json(self):
        json_to_try = JsonManager.yield_json(
            Setting.SYNTHETIC_DATA_INVALID_TYPE, self.customer)
        self.assertFalse(
            JsonManager.is_valid_json(json_to_try),
            msg=f"Testing if JSON for {Setting.SYNTHETIC_DATA_INVALID_TYPE} is not Valid")
    ...
    def test_assert_false_has_data_path(self):
        self.assertFalse(JsonManager.has_data(Setting.SYNTHETIC_DATA_INVALID_TYPE,
                                              Setting.SYNTHETIC_DATA_UUID),
                            msg="Testing if the Path has not Valid data")

    def test_assert_is_instance_load_data(self):
        for abs_type in AbstractionType:
            full_path = (JsonManager.get_path(abs_type) +
                         Setting.SYNTHETIC_DATA_UUID +
    ...
    def test_assert_is_not_instance_load_data(self):
        ...
        full_path = (JsonManager.get_path(Setting.SYNTHETIC_DATA_INVALID_TYPE) +
                     Setting.SYNTHETIC_DATA_UUID + Setting.FILE_EXTENSION)
        ...
        for abs_type in AbstractionType:
            if abs_type is AbstractionType.CUSTOMER:
                self.assertIsInstance(
                                      JsonManager.retrieve_data(abs_type,
                                      Setting.SYNTHETIC_DATA_UUID), Customer)
            elif abs_type is AbstractionType.HOTEL:
                self.assertIsInstance(
                                      JsonManager.retrieve_data(abs_type,
                                      Setting.SYNTHETIC_DATA_UUID), Hotel)
            else:
                # Only on this case the Reservation is not needed to be implemented
                pass

    def test_assert_is_not_instance_retrieve_data(self):
        self.assertIsNone(JsonManager.retrieve_data(Setting.SYNTHETIC_DATA_INVALID_TYPE,
                                                    Setting.SYNTHETIC_DATA_UUID))
        for abs_type in AbstractionType:
            if abs_type is AbstractionType.CUSTOMER:
                self.assertIsNone(JsonManager.retrieve_data(AbstractionType.CUSTOMER,
                                                            Setting.SYNTHETIC_DATA_UUID_WRONG))
            elif abs_type is AbstractionType.HOTEL:
                self.assertIsNone(JsonManager.retrieve_data(AbstractionType.HOTEL,
                                                            Setting.SYNTHETIC_DATA_UUID_WRONG))
            else:
                # Only on this case the Reservation is not needed to be implemented
                pass
